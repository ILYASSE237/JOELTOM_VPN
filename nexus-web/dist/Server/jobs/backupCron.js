 ...
 * BUG FIX: ancienne version pointait sur '/etc/joeltom-tunnel-web/data.db'
 * qui ne correspondait ni au volume Docker ni au chemin db.ts.
 * Standardisé avec JOELTOM_DB_DIR pour cohérence totale.
 */
...
const DB_PATH = path_1.default.join(DB_DIR, 'joeltom.db');
const BACKUP_DIR = process.env.JOELTOM_BACKUP_DIR || '/tmp/joeltom_backups';
...
async function runBackup() {
    try {
        ...
        const backupFile = path_1.default.join(BACKUP_DIR, `joeltom-backup-${timestamp}.db`);
        ...
        const files = fs_1.default.readdirSync(BACKUP_DIR)
            .filter(f => f.startsWith('joeltom-backup-') && f.endsWith('.gz'))
