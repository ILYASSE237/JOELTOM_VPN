...
// ─── BUG FIX: CONFIG_FILE path now uses JOELTOM_DB_DIR to stay consistent ──
const DB_DIR = process.env.JOELTOM_DB_DIR || process.env.NEXUS_DB_DIR || '/etc/joeltom-web';
const CONFIG_FILE = process.env.JOELTOM_CONFIG || path_1.default.join(DB_DIR, 'config.json');
let config = {};
...
const adminUser = config.admin_user || process.env.NEXUS_ADMIN_USER || '';
const adminPass = config.admin_password || process.env.NEXUS_ADMIN_PASS || '';
if (!adminUser || !adminPass || adminPass.length < 8) {
    throw new Error('[SECURITY] NEXUS_ADMIN_USER/NEXUS_ADMIN_PASS must be configured and the password must contain at least 8 characters.');
}
(0, db_1.getDb)();
...
function tryListen(portList, idx) {
    ...
    app.listen(port)
        .on('listening', () => {
        console.log(`[JOELTOM-WEB] ✓ Server running on http://0.0.0.0:${port}`);
        console.log(`[JOELTOM-WEB] ✓ Swagger docs: http://0.0.0.0:${port}/api/docs`);
        console.log(`[JOELTOM-WEB] ✓ Admin user: ${adminUser}`);
        console.log(`[JOELTOM-WEB] ✓ DB: ${path_1.default.join(DB_DIR, 'joeltom.db')}`);
        ...
        if (err.code === 'EADDRINUSE') {
            console.warn(`[JOELTOM-WEB] Port ${port} in use, trying next...`);
            tryListen(portList, idx + 1);
        }
        else {
            console.error('[JOELTOM-WEB] Server error:', err);
            process.exit(1);
        }
    });
}
