...
EOF
  ...
  cd ..
  npm run build
  echo "[nexus-web] Build complete. Copying files to /opt/joeltom-web (requires sudo)"
  sudo mkdir -p /opt/joeltom-web
  sudo cp -r "$PWD"/* /opt/joeltom-web/
  echo "[nexus-web] Installation finished. You can start the service with: sudo systemctl start joeltom-web"
  exit 0
else
