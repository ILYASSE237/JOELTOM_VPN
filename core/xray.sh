clear
# BUG FIX: SERVER_HOST was pointing to the old 'RootNexTPro/joeltomvpn-ScriptAll' repo.
# Updated to use JOELTOM VPN repo. Replace YOUR_GITHUB with your GitHub username/org.
...
# which is set by joeltom.sh (default: Africa/Douala). Falls back to Africa/Douala
...
mkdir -p /etc/nginx
if wget -q -O /tmp/nginx-joeltom.conf "${SERVER_HOST}/module/nginx.conf"; then
install -m 0644 /tmp/nginx-joeltom.conf /etc/nginx/nginx.conf
sleep 3
domain=$(cat /etc/xray/domain)
