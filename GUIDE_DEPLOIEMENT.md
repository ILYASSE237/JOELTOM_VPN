...
Cette commande télécharge et exécute automatiquement `joeltom.sh` qui installe tous les composants VPN (SSH, Xray VLESS/VMESS/Trojan, SlowDNS, UDP).
...
- copier l’application dans /opt/joeltom-web,
...
```bash
sudo systemctl status joeltom-web
...
- copie le projet dans `/opt/joeltom-web`,
- crée et démarre le service systemd `joeltom-web`.
...
# 4. Vérifier les logs
docker-compose logs -f joeltom-web
```
...
### 6. Bot Deploy — VPS cibles (`joeltom_deploy_bot/config.example.json`)
```json
{
...
### 7. Bot Core Telegram (`joeltom_core_bot/config.example.json`)
...
# PM2 (sans Docker)
pm2 status
pm2 logs joeltom-web
pm2 restart joeltom-web

# Docker
docker-compose ps
docker-compose logs -f
docker-compose restart joeltom-web
docker-compose down && docker-compose up -d

# Base de données (SQLite)
sqlite3 /etc/joeltom-web/joeltom.db ".tables"
sqlite3 /etc/joeltom-web/joeltom.db "SELECT * FROM admins;"
```
