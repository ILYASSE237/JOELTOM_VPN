#!/bin/bash
#
# JOELTOM VPN - Shell Script Validator
...
echo -e "${BLUE}╔════════════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║${NC} JOELTOM VPN - Shell Script Validator                    ${BLUE}║${NC}"
echo -e "${BLUE}╚════════════════════════════════════════════════════════════╝${NC}"
echo ""
...
check_typos() {
    local file="$1"
    if grep -nE "meu_option|menu_optin|menu_opton" "$file" > /dev/null 2>&1; then
        echo -e "${RED}✗${NC} $file - Typo détecté"
        grep -nE "meu_option|menu_optin|menu_opton" "$file" | sed "s/^/  /"
        return 1
    fi
    return 0
}
...
for shell_file in $(find "$SCRIPT_DIR" -name "*.sh" -type f); do
    ...
    # Vérifications
    if ! check_ansi_codes "$shell_file"; then ERRORS_FOUND=$((ERRORS_FOUND + 1)); fi
    if ! check_bash_syntax "$shell_file"; then ERRORS_FOUND=$((ERRORS_FOUND + 1)); fi
    if ! check_utf8_encoding "$shell_file"; then ERRORS_FOUND=$((ERRORS_FOUND + 1)); fi
    if ! check_typos "$shell_file"; then ERRORS_FOUND=$((ERRORS_FOUND + 1)); fi
