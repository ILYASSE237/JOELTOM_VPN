#!/usr/bin/env python3
"""
JOELTOM VPN — Bot Deploy Multi-VPS v2
Déploie JOELTOM VPN sur plusieurs VPS en parallèle via SSH Paramiko
Contact: @ILYASSE237
"""
...
logger = logging.getLogger('joeltom-deploy')

CONFIG_PATH = os.environ.get('JOELTOM_DEPLOY_CONFIG', '/etc/joeltom_deploy_bot/config.json')
...
INSTALL_URL = cfg.get('install_url', 'https://raw.githubusercontent.com/joeltom-tech/JOELTOM_VPN/main/autoinstall.sh')

if not BOT_TOKEN:
    raise SystemExit('Missing Telegram bot token. Set JOELTOM_DEPLOY_CONFIG or /etc/joeltom_deploy_bot/config.json')

bot = telebot.TeleBot(BOT_TOKEN, parse_mode='HTML')

# VPS registry: stored in memory + config file
vps_registry: dict[str, dict] = {}
if os.path.exists('/etc/joeltom_deploy_bot/vps_registry.json'):
    with open('/etc/joeltom_deploy_bot/vps_registry.json') as f:
        vps_registry = json.load(f)

def save_registry():
    os.makedirs('/etc/joeltom_deploy_bot', exist_ok=True)
    with open('/etc/joeltom_deploy_bot/vps_registry.json', 'w') as f:
...
BANNER = "🚀 <b>JOELTOM VPN Deploy Bot</b>\n📲 @ILYASSE237"
...
def deploy_to_vps(host, port, user, password, ssh_key, vps_name, chat_id, msg_id):
    """Deploy JOELTOM VPN on a single VPS with live streaming"""
    try:
        ...
        cmd = f"wget -qO /tmp/joeltom_install.sh '{INSTALL_URL}' && bash /tmp/joeltom_install.sh 2>&1"
...
if __name__ == '__main__':
    logger.info("JOELTOM Deploy Bot v2 starting...")
