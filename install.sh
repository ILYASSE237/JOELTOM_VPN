...
if [[ ${#EXISTING_USERS[@]} -gt 0 ]]; then
    ...
    rm -f /etc/sysctl.d/99-z-JOELTOM.conf
...
SYSCTLEOF
    ...
    # Eliminar banners
    rm -f /etc/profile.d/JOELTOM-banner.sh
    rm -f /etc/issue.net
...
if [[ ! -f "$STUNNEL_PEM" ]]; then
    run_cmd "Generando certificado stunnel" "$LINENO" "
        openssl req -x509 -nodes -days 3650 -newkey rsa:2048 \
            ...
            -subj '/CN=stunnel/O=JOELTOM' 2>/dev/null
...
cat > /etc/sysctl.d/99-z-JOELTOM.conf << 'EOF'
...
[Unit]
Description=JOELTOM VPS - Restaurar config de red al arranque
...
if [[ -t 0 ]]; then
    ...
    echo ""
    read -p "🔤 Nom de la marque (Entrée='JOELTOM VPS'): " BRAND_NAME
    read -p "🛡️  Emoji (Entrée='🛡️'): " BRAND_EMOJI
    read -p "💬 Slogan (Entrée='VPS PREMIUM'): " BRAND_SLOGAN
    ...
    BRAND_NAME="${BRAND_NAME:-JOELTOM VPS}"
    BRAND_EMOJI="${BRAND_EMOJI:-🛡️}"
    BRAND_SLOGAN="${BRAND_SLOGAN:-VPS PREMIUM}"
    BRAND_WEB="${BRAND_WEB:-}"
...
step "Installation des modules JOELTOM VPS depuis le dossier local..."

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" 2>/dev/null && pwd -P)"
if [[ -d "$SCRIPT_DIR/protocolos" && -d "$SCRIPT_DIR/usuarios" && -f "$SCRIPT_DIR/menu.sh" ]]; then
    run_cmd "Copie des fichiers JOELTOM VPS" "$LINENO" "mkdir -p /etc/movivip; cp -a "$SCRIPT_DIR"/. /etc/movivip/; chmod -R +x /etc/movivip"
else
    echo -e "${RED}✖ Structure du projet introuvable.${RESET}"
    echo -e "${WHITE}  Lancez install.sh depuis le dossier complet du projet JOELTOM VPS.${RESET}"
    exit 1
fi

run_cmd "Correction CRLF" "$LINENO" "find /etc/movivip -name '*.sh' -type f -exec sed -i 's/\r$//' {} + 2>/dev/null"
...
[Unit]
Description=JOELTOM Dropbear Multi-Port
After=network.target
...
[Unit]
Description=UDP Custom Server JOELTOM
After=network.target
...
[Unit]
Description=JOELTOM VPS State - consumo de red
After=network.target
...
cat > /etc/profile.d/JOELTOM-banner.sh << 'BEOF'
#!/bin/bash
[[ $- != *i* ]] && return
clear
SERVER=$(hostname)
...
center "🛡SISTEMA PROTEGIDO POR JOELTOM VPS🛡"
center ""
BEOF
...
<font color='#29b6f6'>════════════════════════════</font><br><br>
<font color="#00ffff"><small><i>🛡SISTEMA PROTEGIDO POR JOELTOM VPS🛡</i></small></font>
</span></div>
</body>
</html>
IEOF
