...
if [ -d "$SCRIPT_DIR/../joeltom_core_bot" ]; then
  cp -r "$SCRIPT_DIR/../joeltom_core_bot/"* /etc/joeltom_bot/ 2>/dev/null || true
  cp -r "$SCRIPT_DIR/../joeltom_core_bot/modules" /etc/joeltom_bot/ 2>/dev/null || true
else
  cd /tmp
  rm -rf repo_temp
  git clone https://github.com/joeltom-tech/JOELTOM_VPN.git repo_temp >/dev/null 2>&1
  if [ -d repo_temp/joeltom_core_bot ]; then
    cp -r repo_temp/joeltom_core_bot/* /etc/joeltom_bot/ 2>/dev/null || true
  fi
  rm -rf repo_temp
fi
...
ExecStart=/usr/bin/python3 /etc/joeltom_bot/joeltom_bot.py
Restart=always
RestartSec=3
