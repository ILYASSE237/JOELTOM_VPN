...
SELLO='<font color="#00ffff"><small><i>🛡SISTEMA PROTEGIDO POR JOELTOM VPS🛡</i></small></font>'
...
force_sello() {
    ...
    sed -i '/PROTEGIDO POR JOELTOM VPS/d' "$BANNER" 2>/dev/null
...
[[ -z "$SERVER" ]] && SERVER="${SERVER_NAME:-JOELTOM VPN}"

read -rp "$(echo -e "${GREEN}Texto Promocional:${RESET} ")" PROMO
[[ -z "$PROMO" ]] && PROMO="🔥 Bienvenido a $SERVER 🔥"

read -rp "$(echo -e "${GREEN}Canal Telegram (ej. @JOELTOM):${RESET} ")" CHANNEL
...
<br><br>
<font color='#FFD700'><big><big>🛡️ ${SERVER_NAME:-JOELTOM VPN} 🛡️</big></big></font><br>
<font color='#29b6f6'>════════════════════════════</font><br><br>
...
EOF
    ...
    echo
    echo -e "${GREEN}✔ Banner eliminado. Se mantiene el sello de protección JOELTOM VPS.${RESET}"
    ;;
