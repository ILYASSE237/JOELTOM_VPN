#!/bin/bash
# ============================================================
#   JOELTOM VPN — Main Installer (joeltom.sh)
#   Remplace: nexus.sh
# ============================================================
...
BANNER
    echo -e "${NC}"
    echo -e "${BLUE}┏━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┓${NC}"
    echo -e "${BLUE}┃${NC} ${BG_BLUE}          JOELTOM VPN — Auto-Installer v${JOELTOM_VERSION}       ${NC} ${BLUE}┃${NC}"
    echo -e "${BLUE}┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛${NC}"
    echo ""
}
...
check_arch() {
    case "$(uname -m)" in
        x86_64|amd64|aarch64|arm64) log_success "Architecture supportée: $(uname -m)" ;;
        *) log_error "Architecture non supportée: $(uname -m)."; exit 1 ;;
    esac
}

check_internet() {
    if ! curl -fsS --max-time 8 https://raw.githubusercontent.com >/dev/null 2>&1; then
        log_error "Connexion Internet/GitHub indisponible. Vérifiez le réseau avant de continuer."
        exit 1
    fi
...
check_os_version() {
    . /etc/os-release
    if [[ "$ID" == "ubuntu" ]]; then
        if [ "${VERSION_ID%%.*}" -lt 20 ]; then log_error "Ubuntu ${VERSION_ID} trop ancien. Ubuntu 20.04+ requis."; exit 1; fi
    elif [[ "$ID" == "debian" ]]; then
        if [ "${VERSION_ID%%.*}" -lt 11 ]; then log_error "Debian ${VERSION_ID} trop ancien. Debian 11+ requis."; exit 1; fi
    else
        log_error "OS non supporté: ${ID}. Priorité Ubuntu 20.04+ / Debian 11+."; exit 1
    fi
}

check_port_conflicts() {
    command -v ss >/dev/null 2>&1 || return 0
    local ports=(22 53 80 88 109 110 143 443 1194 2087 2200 3128 36712)
    log_step "Vérification des ports avant installation..."
    for p in "${ports[@]}"; do
        if ss -lntup 2>/dev/null | grep -qE ":${p}\b"; then
            log_warn "Port ${p} déjà utilisé. Le processus sera conservé; vérifiez les conflits avant d'activer un protocole qui l'utilise."
        fi
    done
}
...
prepare_env() {
    log_step "Préparation de l'environnement JOELTOM VPN..."
    mkdir -p /etc/xray /etc/joeltom
    touch /etc/xray/domain
    echo "$JOELTOM_VERSION" > /etc/joeltom/version
...
show_tns() {
    ...
    echo -e "${BLUE}┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛${NC}"
    echo -e "${BLUE}┏━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┓${NC}"
    echo -e "${BLUE}┃${NC} ${GREEN}Bienvenue dans JOELTOM VPN!${NC}"
    echo -e "${BLUE}┃${NC}"
    echo -e "${BLUE}┃${NC} ${WHITE}[*] Lisez attentivement les conditions ci-dessous${NC}"
    echo -e "${BLUE}┃${NC} ${WHITE}[*] JOELTOM VPN est fourni tel quel, sans garantie.${NC}"
    echo -e "${BLUE}┃${NC} ${WHITE}[*] N'utilisez pas ce service pour des activités illégales.${NC}"
    echo -e "${BLUE}┃${NC} ${WHITE}[*] JOELTOM VPN n'est pas responsable des pertes de données.${NC}"
...
add_domain() {
    ...
    sleep 3
    log_step "Démarrage de l'installation complète JOELTOM VPN..."
    sleep 2
}
...
run_scripts() {
    ...
    for script in "${scripts[@]}"; do
        ...
        if wget -q "$url" -O "/tmp/${script}"; then
            ...
            if bash "/tmp/${script}"; then
                log_success "${script} terminé."
            else
                log_error "${script} a échoué. Installation interrompue pour éviter un faux état 'OK'."
                exit 1
            fi
        else
            echo -e "${RED}✘${NC}"
...
set_version() {
    echo "$JOELTOM_VERSION" > /etc/joeltom/version
    echo "$JOELTOM_VERSION" > /etc/version
    wget -q "$SERVER_HOST/port_info" -O /etc/xray/port_info 2>/dev/null || true
    log_success "Version enregistrée: $JOELTOM_VERSION"
}
...
joeltom_completed() {
    ...
    echo -e "${GREEN}┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛${NC}"
    echo -e "${GREEN}┏━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┓${NC}"
    echo -e "${GREEN}┃${NC} ${GREEN}✔ JOELTOM VPN est prêt à l'emploi!${NC}"
    echo -e "${GREEN}┃${NC}"
    echo -e "${GREEN}┃${NC} ${WHITE}Domaine  : ${domain}${NC}"
    echo -e "${GREEN}┃${NC} ${WHITE}IP VPS   : ${MYIP}${NC}"
    echo -e "${GREEN}┃${NC} ${WHITE}Version  : ${JOELTOM_VERSION}${NC}"
    echo -e "${GREEN}┃${NC}"
    ...
    echo -e "${GREEN}┃${NC}"
    echo -e "${GREEN}┃${NC} ${WHITE}Script by JOELTOM TEAM${NC}"
    echo -e "${GREEN}┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛${NC}"
    echo -e "${GREEN}●━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━●${NC}"
}
...
main() {
    ...
    joeltom_completed
    cleanner
    if [ "${JOELTOM_NO_REBOOT:-0}" = "1" ]; then
        log_success "Redémarrage automatique désactivé (JOELTOM_NO_REBOOT=1)."
    else
